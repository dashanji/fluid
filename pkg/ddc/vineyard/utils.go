...
func (e *VineyardEngine) getDataSetFileNum() (string, error) {
...
// func (e *VineyardEngine) getConfigMap(name string, namespace string) (configMap *corev1.ConfigMap, err error) {
...
func (e *VineyardEngine) getMasterPodInfo() (podName string, containerName string) {
...
func (e *VineyardEngine) getVineyarddName() (dsName string) {
	return e.name + "-vineyardd"
}
...
//func (e *VineyardEngine) getRunningPodsOfDaemonset(dsName, namespace string) (pods []corev1.Pod, err error) {
//
...
func (e *VineyardEngine) getInitUserDir() string {
...
/*func (e *VineyardEngine) getInitTierPathsEnv(runtime *datav1alpha1.VineyardRuntime) string {
...
func (e *VineyardEngine) parseRuntimeImage(image string, tag string, imagePullPolicy string, imagePullSecrets []v1.LocalObjectReference) (string, string, string, []v1.LocalObjectReference) {
	if len(imagePullPolicy) == 0 {
		imagePullPolicy = common.DefaultImagePullPolicy
	}

	if len(image) == 0 {
		image = docker.GetImageRepoFromEnv(common.VineyardRuntimeImageEnv)
		if len(image) == 0 {
			runtimeImageInfo := strings.Split(common.DefaultVineyardRuntimeImage, ":")
	...
	if len(tag) == 0 {
		tag = docker.GetImageTagFromEnv(common.VineyardRuntimeImageEnv)
		if len(tag) == 0 {
			runtimeImageInfo := strings.Split(common.DefaultVineyardRuntimeImage, ":")
...
func (e *VineyardEngine) GetMetadataInfoFile() string {
	return cdatabackup.AlluxioBackupPathPod + "/" + e.GetMetadataInfoFileName()
}

func (e *VineyardEngine) GetMetadataFileName() string {
	return "metadata-backup-" + e.name + "-" + e.namespace + ".gz"
}

func (e *VineyardEngine) GetMetadataInfoFileName() string {
...
func (e *VineyardEngine) GetWorkerUsedCapacity() (map[string]int64, error) {
	// 2. run clean action
